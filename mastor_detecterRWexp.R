AllMooringsExp<-c("BS15_AU_02a,BS14_AU_04,AW12_AU_BS3,BS13_AU_04,BS16_AU_02a,BS15_AU_02b,AW14_AU_BS3,AL16_AU_BS1,BS13_AU_02a,BS16_AU_05,BS15_AU_04,AW14_AU_BS2")
AllFilesExp<-c("1:104,full,full,full,1:175,full,1:160,full,full,full,301:417,full")
...
sequence<- sample(1:length(AllMooringsExp),length(AllMooringsExp), replace=FALSE)
...
after_model_write <-function(mdata){
  ...
  #now do it for each species individually and each unique mooring ID. Compare with GTtot if available. 
    for(s in 1:length(mSpec)){
      ...
      if(any(paste(mSpec[s],unique(MoorInfo[,10])[v])==TPtottab$MoorCor)){
        ...
        #experiment
        if(whichRun=="GT"){
        detecEvalExp<-read.csv(paste(outputpath,"AddDataSummaryRW_automate.csv",sep=""))
        detecEvalExp<-detecEvalExp[0,]
        detecEvalExp<-data.frame(lapply(detecEvalExp,as.character),stringsAsFactors = FALSE)
        
        detecEvalExp[1,]<-c(experiment,length(indexxx),"GT","RW",paste(MoorVar1[1,"MooringID"]),TPR,FPR,AUCadj)
        
        detecEvalExp2<-read.csv(paste(outputpath,"AddDataSummaryRW_automate.csv",sep=""))
        detecEvalExpFinal<-rbind(detecEvalExp2,detecEvalExp)
        write.csv(detecEvalExpFinal,paste(outputpath,"AddDataSummaryRW_automate.csv",sep=""),row.names=FALSE)
  ...
  MoorVar1 <-MoorVar1[which(paste(mSpec[[s]],MoorVar1$MooringID) %in% TPtottab$MoorCor),]
    ...
    if(whichRun=="GT"){
      ...
      detecEvalExp[1,]<-c(experiment,length(indexxx),"ALL","RW","all",TPR,FPR,AUCadj)
...
if(compareFinal_w_GT=="y"){
  ...
  for(s in mSpec){
    ...
    #save stats and parameters to excel file
    detecEval<-detecEvalFinal[0,]
    detecEval[1,]<-c(s,"all",paste(detnum,paste(detlist2,collapse="+"),sep=";"),"spread",runname,detTotal,numTP,numFP,numFN,TPR,FPR,TPdivFP,AUCadj,NA,NA,NA,paste(allowedZeros,collapse=","),paste(grpsize,collapse=","),"depreciated",paste(detskip,collapse=","),paste(groupInt,collapse=","),NA,timediffself,paste(Mindur,Maxdur,sep=","),as.character(paste(detnum,sum(detlist),sep=";")),FO,LMS," ")
    ...
    detecEvalExp[1,]<-c(experiment,length(indexxx),"NEW","RW",paste(MoorInfo[,10]),TPR,FPR,AUCadj)
